...
class Jobs_Model extends CI_Model
{
     ...
     */
    public function get_medicines_purchased_stock($medicine_id)
    {
        $this->db->select('SUM(' . $this->_table_medicine_stock . '.quantity) as quantity');
        $this->db->select(array($this->_table_medicine_category . '.medicine_category_id'));
        ...
        $this->db->where($this->_table_medicine_category . '.medicine_category_id', $medicine_id);
     ...
     */
    public function get_medicines_sold_stock($medicine_id)
    {
        $this->db->select('SUM(quantity) as quantity');
        $this->db->select(array('medicine_id'));
        $this->db->where($this->_table_medicine_sale_details . '.medicine_id', $medicine_id);
        $data = $this->db->get($this->_table_medicine_sale_details)->result_array();
        
        return $data;
    }
    
    public function update_medicine_stock_status($update_data, $medicine_id)
    {
        $this->db->where('medicine_category_id', $medicine_id);
        $this->db->update($this->_table_medicine_category, $update_data);
    }
}